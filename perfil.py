...
from dados_mock import ranking_jornada
...
def show_perfil_screen(page: ft.Page, nome_usuario: str = "") -> None:
    ...
    opcoes_rankeadas = ranking_jornada()
    ...
    boxes_perfil: list[ft.Container] = []
    for idx, (txt, votos, percentual) in enumerate(opcoes_rankeadas):
        fill_width = max(10, int((percentual / 100) * 320))
        destaque = idx == 0
        cor_fill = ft.Colors.CYAN_ACCENT_400 if destaque else ft.Colors.with_opacity(0.45, ft.Colors.CYAN_ACCENT_400)
                        ...
                        ft.Container(
                            ...
                            content=ft.Row(
                                ...
                                controls=[
                                    ...
                                    ft.Text(f"{percentual:.1f}%", color=ft.Colors.WHITE, size=12, weight=ft.FontWeight.W_600),
                                ],
                            ),
                        ),
                    ]
                ),
                tooltip=f"{votos} votos",
            )
        )
